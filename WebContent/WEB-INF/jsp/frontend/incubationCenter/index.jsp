...
<script>
	function funcBtnSetting() {
		$(".a-showDetail").click(function(event) {
			...
			var url = $(this).parents("tr").find(".detail-url").val();
...
<script>	
	function paggingSetting() {
		...
		var pageIndex = parseInt('<s:property value="incubationCenterPagedList.pageIndex"/>');
		var pageNumber = parseInt('<s:property value="incubationCenterPagedList.pageNumber"/>');
		var pageCount = parseInt('<s:property value="incubationCenterPagedList.pageCount"/>');
...
<!-- 	<h2 class="itemTitle">編輯管理</h2> -->
	<s:form action="index" method="post" validate="true" >
		<div>
			<ul>
				<li class="half">
					<s:textfield placeholder="學校/單位名稱" name="searchCondition.name" maxlength="500" />
				</li>				
				<li class="quarter">
		...
		<div class="page">
			<s:set var="pgList" value="incubationCenterPagedList"/>
