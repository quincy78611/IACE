...
<script>	
	function paggingSetting() {
		...
		var pageIndex = parseInt('<s:property value="subscriberPagedList.pageIndex"/>');
		var pageNumber = parseInt('<s:property value="subscriberPagedList.pageNumber"/>');
		var pageCount = parseInt('<s:property value="subscriberPagedList.pageCount"/>');
...
<body>
	<s:form action="index" method="post" validate="true" >
		<div>
			<ul>
				<li class="half">
					<s:textfield placeholder="姓名, 電話, Email" name="searchCondition.searchText" maxlength="100" />
				</li>
				<li class="quarter"> 
					<s:select name="searchCondition.subScribeState" list='#{"true":"訂閱", "false":"退閱"}' headerKey="" headerValue="---狀態---"/>
				</li>
				<li class="quarter">
		...
		<div class="page">
			<s:set var="pgList" value="subscriberPagedList"/>
		...
		<div class="">
			<table>
				<tr>
					...
					<th nowrap width="">狀態</th>
					<th nowrap width="">功能</th>
				</tr>
				<s:if test="subscriberPagedList != null">
					<s:iterator value="subscriberPagedList.list" status="stat">
						<tr>
							<td>
								<s:property value="%{subscriberPagedList.itemStart + #stat.count -1}" />
								<%-- <s:property value="id" /> --%>
							</td>
							...
							<td><s:property value="%{ isSubscribe ? '訂閱' : '退閱'}" /></td>
							<td>
								<!-- 檢視 -->
