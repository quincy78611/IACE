...
<script type="text/javascript">
	$(document).ready(function () {			
		...
		$(".btn-confirm").click(function(){
			...
			$("form#form-index").attr('action', '<s:url value="patentIndex.action"/>'); // 要把action改為原本的，否則如果使用者按下瀏覽器的上一頁回到目前這個列表頁再去按搜尋就會跑到已經被改變的action所指定的那一頁
		});
		
		$("input[type='checkbox'][name='template.patentIds']").change(function(){
			if (this.checked == false) {
				var value = $(this).val();
				$("#hidden-template-value input[type=hidden][name='template.patentIds'][value='"+value+"']").remove();
			}
		});
	});
</script>
...
<meta name="funcPathText" content="新增 > 專利資料設定 "/>
</head>
<body>
	<s:form action="patentIndex" method="post" validate="true" id="form-index">
		...
		<div class="subForm">
			<div>
				<ul>
					<li class="all">
						<s:textfield placeholder="專利名稱" name="patentSearchCondition.name" maxlength="300" />
					</li>
					<li class="half">
						<s:textfield placeholder="專利權人" name="patentSearchCondition.assignee" maxlength="500" />
					</li>				
					<li class="quarter">
						<s:textfield placeholder="申請日(起)" name="patentSearchCondition.applicationDateS" maxlength="10" cssClass="calendarBox" />
					</li>
					<li class="quarter">
						<s:textfield placeholder="申請日(迄)" name="patentSearchCondition.applicationDateE" maxlength="10" cssClass="calendarBox" />
					</li>
					<li class="quarter">
						<s:textfield placeholder="申請號" name="patentSearchCondition.appliactionNo" maxlength="100" />
					</li>
					<li class="quarter">
						<s:select name="patentSearchCondition.countryCode" list="optionCountryList" listKey="code" listValue="%{code+' - '+name}" headerKey="" headerValue="請選擇申請國" />
					</li>
					<li class="quarter">
						<s:select name="patentSearchCondition.techFieldId" list="techFieldList" listKey="id" listValue="name" headerKey="-1" headerValue="請選擇專利技術領域" />
					</li>
					<li class="quarter">
				...
				<table>
					...
					<s:if test="patentPagedList != null">
						<s:iterator value="patentPagedList.list" status="stat">
							<tr>
								<td>
									<s:if test="template.patentIds.contains(id)">
										<input type="checkbox" name="template.patentIds" value="<s:property value="id"/>" checked="checked"/>
									</s:if>
									<s:else>
										<input type="checkbox" name="template.patentIds" value="<s:property value="id"/>"/>
									</s:else>
								</td>
								<td>
			...
			<div class="page">
				<s:hidden id="pageIndex" name="patentSearchCondition.pageIndex" />
				<s:hidden id="pageSize" name="patentSearchCondition.pageSize" />
							
				<s:set var="pgList" value="patentPagedList"/>
				<s:set var="pgIndex" value="patentSearchCondition.pageIndex"/>
