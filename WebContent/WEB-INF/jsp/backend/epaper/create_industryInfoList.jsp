...
<script type="text/javascript">
	$(document).ready(function () {			
		...
		$(".btn-confirm").click(function(){
			...
			$("form#form-index").attr('action', '<s:url value="industryInfoIndex.action"/>'); // 要把action改為原本的，否則如果使用者按下瀏覽器的上一頁回到目前這個列表頁再去按搜尋就會跑到已經被改變的action所指定的那一頁
		});
		
		$("input[type='checkbox'][name='template.industryInfoIds']").change(function(){
			if (this.checked == false) {
				var value = $(this).val();
				$("#hidden-template-value input[type=hidden][name='template.industryInfoIds'][value='"+value+"']").remove();
			}
		});
	});
</script>
<script>	
	function paggingSetting() {
		...
		var pageIndex = parseInt('<s:property value="industryInfoPagedList.pageIndex"/>');
		var pageNumber = parseInt('<s:property value="industryInfoPagedList.pageNumber"/>');
		var pageCount = parseInt('<s:property value="industryInfoPagedList.pageCount"/>');
...
<meta name="funcPathText" content="新增 > 產業情報設定"/>
</head>
<body>
	<s:form action="industryInfoIndex" method="post" validate="true" id="form-index">
		...
		<div class="subForm">
			<div class="">
				<ul class="row">
					<li class="quarter">
						<s:select name="industryInfoSearchCondition.category" list="@iace.entity.industryInfo.IndustryInfo@getCategoryList()" listKey="code" listValue="name" headerKey="" headerValue="--分類--" />
					</li>
					<li class="half">
						<s:textfield placeholder="請輸入標題" name="industryInfoSearchCondition.searchText"/>
					</li>
					<li class="quarter">
					...
					<tbody>
						<s:if test="industryInfoPagedList != null">
							<s:iterator value="industryInfoPagedList.list" status="stat">
								<tr>
									<td>
										<s:if test="template.industryInfoIds.contains(id)">
											<input type="checkbox" name="template.industryInfoIds" value="<s:property value="id"/>" checked="checked"/>
										</s:if>
										<s:else>
											<input type="checkbox" name="template.industryInfoIds" value="<s:property value="id"/>"/>
										</s:else>
									</td>
									<td>
										<s:property value="%{industryInfoPagedList.itemStart + #stat.count -1}" />
									</td>
			...
			<div class="page">
				<s:hidden id="pageIndex" name="industryInfoSearchCondition.pageIndex" />
				<s:hidden id="pageSize" name="industryInfoSearchCondition.pageSize" />
							
				<s:set var="pgList" value="industryInfoPagedList"/>
				<s:set var="pgIndex" value="industryInfoSearchCondition.pageIndex"/>
