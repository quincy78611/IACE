...
<script type="text/javascript">
	$(document).ready(function () {			
		...
		$(".btn-confirm").click(function(){
			...
			$("form#form-index").attr('action', '<s:url value="activityIndex.action"/>'); // 要把action改為原本的，否則如果使用者按下瀏覽器的上一頁回到目前這個列表頁再去按搜尋就會跑到已經被改變的action所指定的那一頁
		});
		
		$("input[type='checkbox'][name='template.activityIds']").change(function(){
			if (this.checked == false) {
				var value = $(this).val();
				$("#hidden-template-value input[type=hidden][name='template.activityIds'][value='"+value+"']").remove();
			}
		});
	});
</script>
<script>	
	function paggingSetting() {
		...
		var pageIndex = parseInt('<s:property value="activityPagedList.pageIndex"/>');
		var pageNumber = parseInt('<s:property value="activityPagedList.pageNumber"/>');
		var pageCount = parseInt('<s:property value="activityPagedList.pageCount"/>');
...
<meta name="funcPathText" content="新增 > 活動人培設定 "/>
</head>
<body>
	<s:form action="activityIndex" method="post" validate="true" id="form-index">
		...
		<div class="subForm">
			<div class="">
				<ul class="row">
					<li class="quarter">
						<s:select name="activitySearchCondition.category" list="@iace.entity.activity.Activity@getCategoryList()" listKey="code" listValue="name" headerKey="" headerValue="--分類--" />
					</li>
					<li class="half">
						<s:textfield placeholder="請輸入標題" name="activitySearchCondition.searchText"/>
					</li>
					<li class="quarter">
					...
					<tbody>
						<s:if test="activityPagedList != null">
							<s:iterator value="activityPagedList.list" status="stat">
								<tr>
									<td>
										<s:if test="template.activityIds.contains(id)">
											<input type="checkbox" name="template.activityIds" value="<s:property value="id"/>" checked="checked"/>
										</s:if>
										<s:else>
											<input type="checkbox" name="template.activityIds" value="<s:property value="id"/>"/>
										</s:else>
									</td>
									<td>
			...
			<div class="page">
				<s:hidden id="pageIndex" name="activitySearchCondition.pageIndex" />
				<s:hidden id="pageSize" name="activitySearchCondition.pageSize" />
							
				<s:set var="pgList" value="activityPagedList"/>
				<s:set var="pgIndex" value="activitySearchCondition.pageIndex"/>
