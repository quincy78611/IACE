...
<script type="text/javascript">
	$(document).ready(function () {			
		...
		$(".btn-confirm").click(function(){
			...
			$("form#form-index").attr('action', '<s:url value="newsIndex.action"/>'); // 要把action改為原本的，否則如果使用者按下瀏覽器的上一頁回到目前這個列表頁再去按搜尋就會跑到已經被改變的action所指定的那一頁
		});
		
		$("input[type='checkbox'][name='template.newsIds']").change(function(){
			if (this.checked == false) {
				var value = $(this).val();
				$("#hidden-template-value input[type=hidden][name='template.newsIds'][value='"+value+"']").remove();
			}
		});
	});
</script>
...
<meta name="funcPathText" content="新增 > 公告訊息設定"/>
</head>
<body>
	<s:form action="newsIndex" method="post" validate="true" id="form-index">
		...
		<div class="subForm">
			<div class="">
				<ul class="row">
					<li class="quarter">
						<s:select name="newsSearchCondition.category" list="@iace.entity.news.News@getCategoryList()" listKey="code" listValue="name" headerKey="" headerValue="--分類--" />
					</li>
					<li class="half">
						<s:textfield placeholder="請輸入標題" name="newsSearchCondition.searchText"/>
					</li>
					<li class="quarter">
					...
					<tbody>
						<s:if test="newsPagedList != null">
							<s:iterator value="newsPagedList.list" status="stat">
								<tr>
									<td>
										<s:if test="template.newsIds.contains(id)">
											<input type="checkbox" name="template.newsIds" value="<s:property value="id"/>" checked="checked"/>
										</s:if>
										<s:else>
											<input type="checkbox" name="template.newsIds" value="<s:property value="id"/>"/>
										</s:else>
									</td>
									<td>
			...
			<div class="page">
				<s:hidden id="pageIndex" name="newsSearchCondition.pageIndex" />
				<s:hidden id="pageSize" name="newsSearchCondition.pageSize" />
							
				<s:set var="pgList" value="newsPagedList"/>
				<s:set var="pgIndex" value="newsSearchCondition.pageIndex"/>
