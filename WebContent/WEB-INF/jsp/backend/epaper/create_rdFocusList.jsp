...
<script type="text/javascript">
	$(document).ready(function () {			
		...
		$(".btn-confirm").click(function(){
			...
			$("form#form-index").attr('action', '<s:url value="rdFocusIndex.action"/>'); // 要把action改為原本的，否則如果使用者按下瀏覽器的上一頁回到目前這個列表頁再去按搜尋就會跑到已經被改變的action所指定的那一頁
		});
		
		$("input[type='checkbox'][name='template.rdFocusIds']").change(function(){
			if (this.checked == false) {
				var value = $(this).val();
				$("#hidden-template-value input[type=hidden][name='template.rdFocusIds'][value='"+value+"']").remove();
			}
		});
	});
</script>
<script>	
	function paggingSetting() {
		...
		var pageIndex = parseInt('<s:property value="rdFocusPagedList.pageIndex"/>');
		var pageNumber = parseInt('<s:property value="rdFocusPagedList.pageNumber"/>');
		var pageCount = parseInt('<s:property value="rdFocusPagedList.pageCount"/>');
...
<meta name="funcPathText" content="新增 > 研發焦點設定"/>
</head>
<body>
	<s:form action="rdFocusIndex" method="post" validate="true" id="form-index">
		...
		<div class="subForm">
			<div class="">
				<ul class="row">
					<li class="quarter">
						<s:select name="rdFocusSearchCondition.category" list="@iace.entity.rdFocus.RdFocus@getCategoryList()" listKey="code" listValue="name" headerKey="" headerValue="--分類--" />
					</li>
					<li class="half">
						<s:textfield placeholder="請輸入標題" name="rdFocusSearchCondition.searchText"/>
					</li>
					<li class="quarter">
					...
					<tbody>
						<s:if test="rdFocusPagedList != null">
							<s:iterator value="rdFocusPagedList.list" status="stat">
								<tr>
									<td>
										<s:if test="template.rdFocusIds.contains(id)">
											<input type="checkbox" name="template.rdFocusIds" value="<s:property value="id"/>" checked="checked"/>
										</s:if>
										<s:else>
											<input type="checkbox" name="template.rdFocusIds" value="<s:property value="id"/>"/>
										</s:else>
									</td>
									<td>
										<s:property value="%{rdFocusPagedList.itemStart + #stat.count -1}" />
									</td>
			...
			<div class="page">
				<s:hidden id="pageIndex" name="rdFocusSearchCondition.pageIndex" />
				<s:hidden id="pageSize" name="rdFocusSearchCondition.pageSize" />
							
				<s:set var="pgList" value="rdFocusPagedList"/>
				<s:set var="pgIndex" value="rdFocusSearchCondition.pageIndex"/>
