<%@ page language="java" contentType="text/html; charset=UTF-8" pageEncoding="UTF-8"%>
<%@ taglib prefix="s" uri="/struts-tags"%>
<%@ taglib prefix="sb" uri="/struts-bootstrap-tags" %>

<div class="row">
	<div class="col-sm-12 col-xs-12">
		<div class="well well-sm">
			<table class="table content_03" style="margin-bottom: 0;">
				<tbody>
					<tr>
						<td width="10%" style="border: none;"><h4>
								<span class="label label-info">研發成果</span>
							</h4></td>
						<td width="10%" style="border: none;" class="date_01">計畫名稱</td>
						<td style="border: none;"><a href="#" class="list_link_01">具高效能驅動之模組式磁浮軸承機電整合控制系統設計與開發 </a></td>
					</tr>
					<tr>
						<td>&nbsp;</td>
						<td class="date_01">主持人</td>
						<td>李政道</td>
					</tr>
					<tr>
						<td>&nbsp;</td>
						<td class="date_01">年度</td>
						<td>100</td>
					</tr>
					<tr>
						<td>&nbsp;</td>
						<td class="date_01">研究領域</td>
						<td>10: 數學類&nbsp;&nbsp;22: 電子電機工程類</td>
					</tr>
					<tr>
						<td>&nbsp;</td>
						<td class="date_01">技術名稱</td>
						<td>高效能驅動之模組式磁浮軸承機電整合控制系統</td>
					</tr>
					<tr>
						<td>&nbsp;</td>
						<td class="date_01">技術簡述</td>
						<td>設計一模組式磁浮軸承裝置直接應用於單軸馬達軸承。<br> 針對磁浮驅動與控制系統利用遞迴步階方式進行控制器設計。
						</td>
					</tr>
				</tbody>
			</table>
		</div>
	</div>
	<div class="col-sm-12 col-xs-12">
		<div class="well well-sm">
			<table class="table content_03" style="margin-bottom: 0;">
				<tbody>
					<tr>
						<td width="10%" style="border: none;"><h4>
								<span class="label label-success">專利資料</span>
							</h4></td>
						<td width="10%" style="border: none;" class="date_01">專利名稱</td>
						<td style="border: none;"><a href="#" class="list_link_01">METHOD OF CONFIGURING MULTI-LEVEL PACKET TRANSMISSION PATHS IN A WIRELESS SENSOR NETWORK </a></td>
					</tr>
					<tr>
						<td>&nbsp;</td>
						<td class="date_01">專利權人</td>
						<td>NAT UNIV TSINGHUA</td>
					</tr>
					<tr>
						<td>&nbsp;</td>
						<td class="date_01">申請號</td>
						<td>US13049636A</td>
					</tr>
					<tr>
						<td>&nbsp;</td>
						<td class="date_01">申請國別</td>
						<td>美國</td>
					</tr>
					<tr>
						<td>&nbsp;</td>
						<td class="date_01">申請日</td>
						<td>2011/8/16</td>
					</tr>
					<tr>
						<td>&nbsp;</td>
						<td class="date_01">狀態</td>
						<td>Application</td>
					</tr>
					<tr>
						<td>&nbsp;</td>
						<td class="date_01">專利技術領域</td>
						<td>IoT</td>
					</tr>
				</tbody>
			</table>
		</div>
	</div>
	<div class="col-sm-12 col-xs-12">
		<div class="well well-sm">
			<table class="table content_03" style="margin-bottom: 0;">
				<tbody>
					<tr>
						<td width="10%" style="border: none;"><h4>
								<span class="label label-primary">合作案例</span>
							</h4></td>
						<td width="10%" style="border: none;" class="date_01">標題</td>
						<td style="border: none;"><a href="#" class="list_link_01">不再鬼影幢幢 雙模式抬頭顯示器有絕招 </a></td>
					</tr>
					<tr>
						<td>&nbsp;</td>
						<td class="date_01">案名</td>
						<td>無鬼影雙模式抬頭顯示系統</td>
					</tr>
					<tr>
						<td>&nbsp;</td>
						<td class="date_01">年度</td>
						<td>2015</td>
					</tr>
					<tr>
						<td>&nbsp;</td>
						<td class="date_01">研發團隊</td>
						<td>國立清華大學 副教授陳政寰<br> （現職國立交通大學光電工程學系）
						</td>
					</tr>
					<tr>
						<td>&nbsp;</td>
						<td class="date_01">類別</td>
						<td>商品化</td>
					</tr>
					<tr>
						<td>&nbsp;</td>
						<td class="date_01">輔導團隊</td>
						<td>工研院技轉中心，工研院生醫所</td>
					</tr>
				</tbody>
			</table>
		</div>
	</div>
	<div class="col-sm-12 col-xs-12">
		<div class="well well-sm">
			<table class="table content_03" style="margin-bottom: 0;">
				<tbody>
					<tr>
						<td width="10%" style="border: none;"><h4>
								<span class="label label-warning">產學人才</span>
							</h4></td>
						<td width="10%" style="border: none;" class="date_01">姓名</td>
						<td style="border: none;"><a href="#" class="list_link_01">廖顯奎</a></td>
					</tr>
					<tr>
						<td>&nbsp;</td>
						<td class="date_01">性別</td>
						<td>男</td>
					</tr>
					<tr>
						<td>&nbsp;</td>
						<td class="date_01">現職單位</td>
						<td>國立臺灣科技大學電子工程系/光電工程研究所/光電科技中心</td>
					</tr>
					<tr>
						<td>&nbsp;</td>
						<td class="date_01">現職職位</td>
						<td>特聘教授</td>
					</tr>
					<tr>
						<td>&nbsp;</td>
						<td class="date_01">合作專長</td>
						<td>光纖通訊；光學感測；LED 量測與應用；智慧財產權</td>
					</tr>
				</tbody>
			</table>
		</div>
	</div>
	<div class="col-sm-12 col-xs-12">
		<div class="well well-sm">
			<table class="table content_03" style="margin-bottom: 0;" border="0">
				<tbody>
					<tr>
						<td width="10%" style="border: none;"><h4>
								<span class="label label-danger">育成中心</span>
							</h4></td>
						<td width="10%" style="border: none;" class="date_01">學校名稱</td>
						<td style="border: none;"><a href="#" class="list_link_01">國立政治大學</a></td>
					</tr>
					<tr>
						<td>&nbsp;</td>
						<td class="date_01">單位名稱</td>
						<td>創新育成中心</td>
					</tr>
					<tr>
						<td>&nbsp;</td>
						<td class="date_01">屬性</td>
						<td>國立大專院校</td>
					</tr>
				</tbody>
			</table>
		</div>
	</div>
	<div class="col-sm-12 col-xs-12">
		<div class="well well-sm">
			<table class="table content_03" style="margin-bottom: 0;" border="0">
				<tbody>
					<tr>
						<td width="10%" style="border: none;"><h4>
								<span class="label label-purple">法規/文獻</span>
							</h4></td>
						<td width="10%" style="border: none;" class="date_01">題名</td>
						<td style="border: none;"><a href="#" class="list_link_01">專科以上學校產學合作實施辦法</a></td>
					</tr>
					<tr>
						<td>&nbsp;</td>
						<td class="date_01">語文</td>
						<td>中文</td>
					</tr>
					<tr>
						<td>&nbsp;</td>
						<td class="date_01">出版年</td>
						<td>2014</td>
					</tr>
				</tbody>
			</table>
		</div>
	</div>
	<div class="col-sm-12 col-xs-12">
		<div class="well well-sm">
			<table class="table content_03" style="margin-bottom: 0;" border="0">
				<tbody>
					<tr>
						<td width="10%" style="border: none;"><h4>
								<span class="label label-pink">其它</span>
							</h4></td>
						<td width="10%" style="border: none;" class="date_01">計畫名稱</td>
						<td style="border: none;"><a href="#" class="list_link_01">具高效能驅動之模組式磁浮軸承機電整合控制系統設計與開發 </a></td>
					</tr>
					<tr>
						<td>&nbsp;</td>
						<td class="date_01">主持人</td>
						<td>李政道</td>
					</tr>
					<tr>
						<td>&nbsp;</td>
						<td class="date_01">年度</td>
						<td>100</td>
					</tr>
					<tr>
						<td>&nbsp;</td>
						<td class="date_01">研究領域</td>
						<td>10: 數學類&nbsp;&nbsp;22: 電子電機工程類</td>
					</tr>
					<tr>
						<td>&nbsp;</td>
						<td class="date_01">技術名稱</td>
						<td>高效能驅動之模組式磁浮軸承機電整合控制系統</td>
					</tr>
				</tbody>
			</table>
		</div>
	</div>
	<div class="col-sm-12 col-xs-12">
		<div class="well well-sm">
			<table class="table content_03" style="margin-bottom: 0;" border="0">
				<tbody>
					<tr>
						<td width="10%" style="border: none;"><h4>
								<span class="label label-pink">活動/人培</span>
							</h4>
							<h4>
								<span class="label label-default"><i class="fa fa-cube right5" aria-hidden="true"></i>成果發表</span>
							</h4></td>
						<td width="10%" style="border: none;" class="date_01">主題</td>
						<td style="border: none;"><a href="#" class="list_link_01">運用法人鏈結產學合作計畫說明會 圓滿達成！</a></td>
					</tr>
					<tr>
						<td>&nbsp;</td>
						<td class="date_01">活動日期</td>
						<td>2016年6月20日</td>
					</tr>
					<tr>
						<td>&nbsp;</td>
						<td class="date_01">活動地點</td>
						<td>政治大學</td>
					</tr>
					<tr>
						<td>&nbsp;</td>
						<td class="date_01">內容</td>
						<td><img src="images/pic_07.jpg" class="img_radius" align="left" vspace="1" hspace="1" style="max-width: 200px; margin-right: 15px;" alt="" />運用法人鏈結產學合作計畫「產學媒合服務團」成員於5月6日初次拜訪國立政治大學進行計畫推廣，獲該校廣大迴響，期望未來工研院與政治大學有更多產學媒合合作機會。故該校方與工研院旋即規畫於6月20日針對政治大學校內教師舉辦計畫說明暨專利諮詢服務會。</td>
					</tr>
				</tbody>
			</table>
		</div>
	</div>
</div>
