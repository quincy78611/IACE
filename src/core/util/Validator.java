 ...
 * </p>
 * <p>
 * Description:身分證字號,IP,Email,門號,關鍵字過濾等檢查功能
 * </p>
 * <p>
 * Copyright: Copyright (c) 2006
 * </p>
 * <p>
 * Company:InfoTech Ltd. http://www.info.com.tw
 * </p>
 * 
...
public class Validator {
	...
	public static boolean isValidTWPID(String twpid) {
		...
		if (TWPID_PATTERN.matcher(twpid.toUpperCase()).matches()) {
			...
			int sum = 0;
			sum = (int) (code / 10) + 9 * (code % 10) + 8 * (twpid.charAt(1) - '0') + 7 * (twpid.charAt(2) - '0') + 6 * (twpid.charAt(3) - '0') + 5 * (twpid.charAt(4) - '0') + 4 * (twpid.charAt(5) - '0') + 3 * (twpid.charAt(6) - '0') + 2 * (twpid.charAt(7) - '0') + 1 * (twpid.charAt(8) - '0') + (twpid.charAt(9) - '0');
			if ((sum % 10) == 0) {
				result = true;
			}
		}
	...
	public static boolean isValidQueryString(String sqlStr) {
		if (sqlStr == null || sqlStr.length() <= 0 || sqlStr.indexOf("@") >= 0 || sqlStr.indexOf("'") >= 0 || sqlStr.indexOf("_") >= 0 || sqlStr.indexOf("\"") >= 0 || sqlStr.indexOf("%") >= 0 || sqlStr.indexOf("=") >= 0) {
			// "關鍵字不可包含 ( @ ' % \ _ = ) 等字元\n";
			return false;
		}
		return true;
